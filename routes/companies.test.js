...
describe("GET /companies", function () {
  ...
  test("ok with filters", async function () {
    const resp = await request(app).get("/companies").query({minEmployees: 1, maxEmployees: 2, name: "c"});
    expect(resp.body).toEqual({
      companies:
          [
            {
  ...
  test("returns 400 status code for invalid search filters", async function () {
    const resp = await request(app).get("/companies").query({minEmployees: 1, maxEmployees: 2, wrongFilter: "c"});
